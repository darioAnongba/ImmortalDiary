...
contract CTToken is Ownable {
    ...
    PublicParameters private publicParameters;
    BulletproofVerifier private bulletproofVerifier;

    /* --- Events --- */
    event Registration(address indexed from, uint256[2] pubKey);
    ...
    */
    ...
    constructor(address _publicParameters, address _bulletproofVerifier) public {
    ...
    */
    function mint(address _to, uint256 _pcTo, uint256[3] _encryptedData) onlyOwner public returns (bool) {
    ...
    */
    function register(uint256[2] _pubKey) public returns (bool) {
        ...
        emit Registration(msg.sender, _pubKey);
        return true;
    }
    ...
    */
    function transfer(
        ...
        uint256[] _utxoIds,
        uint256[6] _encryptedData) commitmentsAreProven([_pcTo, _pcRemaining]) public returns (bool success) {
    
        // Central bank cannot transfer or receive tokens, only mint them for others
        require(msg.sender != owner && _to != owner && _to != address(0));
        success = internalTransfer(_to, _pcTo, _pcRemaining, _utxoIds, _encryptedData);
    }

    /**
    * @dev withdraw tokens. All PCs must have been proved to be positive. A withdraw is basically a transfer to the central bank
    ...
    */
    function withdraw(
        ...
        uint256 _pcRemaining,
        uint256[] _utxoIds,
        uint256[6] _encryptedData) commitmentsAreProven([_pcWithdraw, _pcRemaining]) public returns (bool success) {

        // Central bank cannot withdraw tokens because it should never have tokens in the first place.
